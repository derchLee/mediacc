...
function buildImageLayoutMeta(): ImageLayoutMetaT {
  ...
  return {
    ...
    softwareApp: {
      ...
      description:
        "ブラウザ内で画像の変換・圧縮ができる無料オンラインツール。サーバーへアップロードせず、JPG、PNG、WebP、AVIF形式の変換とファイルサイズ削減が可能。",
      url,
      downloadUrl: url,
      inLanguage: "ja",
...
function buildVideoLayoutMeta(): VideoLayoutMetaT {
  ...
  return {
    ...
    softwareApp: {
      ...
      description:
        "FFmpeg.wasmでブラウザ内で動画の変換・圧縮ができる無料オンラインツール。サーバーへアップロードせず、MP4、WebM、MOV、MKV、AVI形式の変換とファイルサイズ削減が可能。",
      url,
      downloadUrl: url,
      inLanguage: "ja",
      isAccessibleForFree: true,
